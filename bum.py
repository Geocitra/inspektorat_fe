...
class AggregatorConfig:
    ...
    # 3. Whitelist Folder yang Diizinkan untuk Ditelusuri (Next.js / React / TypeScript Frontend)
    ALLOWED_DIRS = {
        "docs", "public", "src", "app", "components", "hooks", "lib", "store",
        "types", "styles", "config", "context", "providers", "utils", "helpers",
        "models", "validators", "tests", "e2e", "modules", "common"
    }

    # 4. Ekstensi Berkas Teks yang Diizinkan untuk Frontend Next.js / React / TypeScript
    INCLUDE_EXTENSIONS = {
        ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".css", ".scss", ".sass",
        ".html", ".json", ".md", ".yaml", ".yml", ".env.example", ".sh", ".sql",
        ".svg"
    }
    ...
    ESSENTIAL_ROOT_FILES = {
        ...
        "next.config.js", "postcss.config.mjs", "components.json", "eslint.config.mjs",
        "tailwind.config.ts", "tailwind.config.js", "docker-compose.yml", "Dockerfile",
    ...
    def execute(self):
        ...
        try:
            with open(self.output_file, "w", encoding="utf-8") as out_file:
                out_file.write("=== GEOAPIP FE CODEBASE BUNDLE ===\n\n")
                ...
                for root, dirs, files in os.walk(self.target_path):
                    ...
                    for file_name in files:
                        ...
                        if is_root_file and file_name not in self.config.ESSENTIAL_ROOT_FILES and file_path.suffix not in self.config.INCLUDE_EXTENSIONS:
                            continue
